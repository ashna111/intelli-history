...
def predictTopic(url):
        ...
        document = cleanAndJoinText(urlToText(url))
...
def cleanAndJoinText(content):
        whitelist = set('abcdefghijklmnopqrstuvwxyz ABCDEFGHIJKLMNOPQRSTUVWXYZ')
        final = ''
        for i in range(len(content)):
                answer = ''.join(filter(whitelist.__contains__, content[i]))
                final = final + answer
...
def summarizePage(url):
        ...
        summary = []
        for i in range(len(long_content)):
                summary.append(findSummary(long_content[i]))
...
def makePDF(paras):
        ...
        while(i<len(paras)):
                ...
                while(j<len(paras[i])):
                        if(j+60<len(paras[i])):
                                pdf.cell(10, 10, paras[i][j:j+60],0,1)
                                j=j+60
                        else:
                                diff=len(paras[i])-j
                                pdf.cell(10, 10, paras[i][j:j+diff],0,1)
                                j=j+diff
                i=i+1
        pdf.output('content.pdf', 'F')
